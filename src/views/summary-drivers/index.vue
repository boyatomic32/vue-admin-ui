<template>
  <div class="app-container">
    <div class="searchForm mt-2">
      <el-row class="pb-5" :gutter="20">
        <el-col :xs="24" :sm="24" :lg="6">
          <el-select v-model="partnerId" class="search-item mb-2">
            <el-option
        ...
        <el-col :xs="24" :sm="24" :lg="10">
          <el-input class="mb-2" :span="24" placeholder="Search....">
            <template #prepend><svg-icon icon-class="search" /></template>
          </el-input>
        </el-col>
        <el-col :xs="24" :sm="24" :lg="8">
          <el-button class="mb-2" type="primary">
            <svg-icon icon-class="search" /> Search
          </el-button>
          <el-button class="mb-2" type="info">
            <i class="el-icon-refresh" /> Clear
          </el-button>
        </el-col>
      </el-row>
    </div>
    <ProTable
      ref="SummaryDriverTable"
      border
      :columns="columns"
      :data="loadData"
    ...
    >
      ...
      <template #place="slotProps">
        <span>{{ slotProps.row.log.place }}</span>
      </template>
      <template #complete="slotProps">
        <span>{{ slotProps.row.log.complete }}</span>
      </template>
      <template #error="slotProps">
        <span>{{ slotProps.row.log.error }}</span>
      </template>
      <template #distance="slotProps">
        <span>{{ slotProps.row.log.distance }}</span>
      </template>
      <template #period="slotProps">
        <span>{{ slotProps.row.log.min_date }}</span>
        <span> ถึง </span>
        <span>{{ slotProps.row.log.max_date }}</span>
      </template>
      ...
      <template #actions>
        <span class="fixed-width">
          <el-button type="success" size="mini">
            <svg-icon icon-class="list" /> รายการ
          </el-button>
          <el-button size="mini" type="danger">
            <svg-icon icon-class="international" /> GPS
          </el-button>
        </span>
      </template>
    </ProTable>
...
import { fetchSummaryList } from '@/api/driver'
...
export default {
  ...
  setup() {
    ...
    function loadData(parameter) {
      ...
      return fetchSummaryList(requestParameters).then((res) => {
        // format data
        return res
      })
    }
